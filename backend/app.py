...
def twitter_status():
    if not ENABLE_TWITTER:
        # Twitter feature disabled; treat as not requiring onboarding
        return {"new": False}
    ...
    response = User.get_user_by_wallet_address(wallet_address)
    if response["error"]:
        User.add_user({"wallet_address": wallet_address})
        return {"new": True}

    user = response.get("data")
    if user and user.get("oauth_token") and user.get("oauth_token_secret"):
        return {"new": False}
    else:
        return {"new": True}
...
def tweet():
    if not ENABLE_TWITTER:
        return {"error": True, "msg": "Twitter feature disabled"}, 501
    data = request.json
